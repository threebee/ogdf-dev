...
*/
void norrosReittu(Graph &G, int n, Array<double> &weights) {
	G.clear();
	if (n == 0) return;
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			double p = 1.0 - exp(-(weights[i] * weights[j]) / Wk);
			if (dist(rng) <= p) {
				G.newEdge(v[i], v[j]);
			}
		}
...
//! Creates a Norros-Reittu Graph with random weights. 
/**
* @param G is assigned the generated graph.
* @param n is the number of nodes of the generated graph.
*/
void norrosReittuRandomWeights(Graph &G, int n) {
	G.clear();
	if (n == 0) return;
...
int main(){
	//! TODO: tests
	Graph G;
	norrosReittuRandomWeights(G, 100);
